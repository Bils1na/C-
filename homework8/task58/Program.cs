...
void MatrixProduct(int[,] matrix_one, int[,] matrix_two)
{
    ...
    for (int i = 0; i < matrix_one.GetLength(0); i++)
    {
        ...
        {
            for (int k = 1; k < matrix_two.GetLength(0); k++)
            {
                matrix_product[i, j] = matrix_one[i, j] * matrix_two[i, j] +
                matrix_one[i, k] * matrix_two[k, i];
            }
            System.Console.Write($"{matrix_product[i, j]} ");
        }
        System.Console.WriteLine();
    }
...
System.Console.WriteLine();
MatrixProduct(matrix_one, matrix_two);